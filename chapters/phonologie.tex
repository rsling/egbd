...
In Fällen mit scheinbar drei Segmenten im Endrand wie \textit{trittst} muss [t] dann auch extrasilbisch sein, das das vorangehende [s] bereits extrasilbisch ist.
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex{\label{ex:diesystematikderraender120} /\textipa{l\u{E}st}/ $\Rightarrow$ \textipa{[lEst]} (\textit{lässt})}
  \end{xlist}
\end{exe}
...
Im Endrand sollte schließlich aufgrund der Endranddesonorisierung kein stimmhafter Plosiv vorkommen können.
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex{\label{ex:einsilblerundzweisilbler140} *\textipa{[a:lt]}}
...
Die strukturell unmöglichen Simplizia *\textipa{[vi:Nk]} und *\textipa{[a:lt]} zeigen.%
\footnote{Der Einsilbler \textit{aalt} \textipa{[a:l+t]} ist möglich, aber eben nicht als Simplex.
