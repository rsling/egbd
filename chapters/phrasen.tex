...
In (\ref{ex:flexibilitaetderkoordination001}) werden einige völlig akzeptable Koordinationsstrukturen gezeigt, in denen die koordinierten Konstituenten (eingeklammert) nicht dieselbe Kategorie (denselben Phrasentyp) haben.

\begin{exe}
  \ex\label{ex:flexibilitaetderkoordination001} 
  \begin{xlist}
    \ex{\label{ex:flexibilitaetderkoordination002} <++>}
    \ex{\label{ex:flexibilitaetderkoordination003} Die Kampfrichterin bewertet [[die Ästhetik des Sprungs] und [ob er korrekt ausgeführt wurde]].}
    \ex{\label{ex:flexibilitaetderkoordination004} [Tania führte ihre Rückwärtssprünge [elegant] und [mit hoher Präzision]] aus.}
    \ex{\label{ex:flexibilitaetderkoordination005} <++>}
  \end{xlist}
\end{exe}

Diese Flexibilität von Koordinationsstrukturen ist im übrigen auch dafür verantwortlich, dass der Koordinationstest so unzuverlässig ist (siehe Abschnitt~\ref{sec:konstituententests}, insbesondere S.~\pageref{abs:koordinationstest}).
Im Vergleich mit den nicht akzeptablen Beispielen in (\ref{ex:flexibilitaetderkoordination006}) wird deutlich, warum die Beispiele in (\ref{ex:flexibilitaetderkoordination001}) funktionieren, und wie Koordinationsstrukturen genauer beschrieben werden könnten.

\begin{exe}
  \ex\label{ex:flexibilitaetderkoordination006} 
  \begin{xlist}
    \ex{\label{ex:flexibilitaetderkoordination007} }
    \ex{\label{ex:flexibilitaetderkoordination008} }
    \ex{\label{ex:flexibilitaetderkoordination009} }
    \ex{\label{ex:flexibilitaetderkoordination010} }
  \end{xlist}
\end{exe}

Eigentlich wäre es also genauer, die 
...
\begin{enumerate}
  ...
  \item keinem oder genau einem links von der Adjektivposition stehenden Kardinalzahlwort (\textit{Kard}),
...
Wie in Vertiefung~\ref{vert:zahlwörter} auf Seite~\pageref{vert:zahlwörter} gezeigt wurde, verhalten sich Kardinalia wie \textit{zwei} und \textit{drei} speziell und bilden strenggenommen eine eigene Klasse nicht flektierbarer Wörter.
...
\begin{exe}
  \ex{\label{ex:diestrukturdernp018} [Der zweite Versuch] erhielt eine hohe Wertung.}
\end{exe}
