...
\begin{enumerate}
  ...
  \item\leavevmode
    \SonDiag[12]{{t͡s/\plo/0, uː/\vok/0, 0/-1/0, z/\fri/0, ɛ/\vok/0, t͡s/\plo/0, 0/-1/0, l/\liq/0, ɪ/\vok/0, 0/-1/0, ç/\fri/0, ə/\vok/0}}\\[\baselineskip]
  \item\leavevmode
...
\begin{enumerate}
  ...
  \item Präposition (\textit{während des Spiels}),\\
    Subjunktion (\textit{während wir spielen})
...
%  \item Satzäquivalent
  ...
  \item Adverb (\textit{Ich kann damit nichts anfangen.}),\\
    Subjunktion (\textit{Ich gebe, damit du gibst.})
  \item Partikel
  \item Adverb
  \item Verb
  \item Substantiv
...
Zu beachten ist, dass die infiniten Ergänzungen von Modalverben (\zB \textit{wählen} in \textit{wird wählen können}) nicht Teil der Tempuskonstruktion sind.
...
\begin{enumerate}
  ...
  \item\leavevmode
    ...
    Mangels Vorfeldfähigkeit kann es keine AvP sein.
...
Wenn der Relativsatz\slash eingebettete w-Satz [\textit{wer dass kommt}] lautet, handelt es sich um eine SP, bei der das Relativ-Element über die Subjunktion hinausbewegt wird.
...
\begin{figure}[!htbp]
  \centering
  \begin{forest}
    [SP, calign=child, calign child=2
      [NP\Sub{1}, tier=preterminal
        [\textit{wer}, narroof]
      ]
      [\textbf{K}, tier=preterminal
  ...
  \caption{Analyse eines Relativsatzes mit Subjunktion}
...
Kommas, die das Ende einer Struktur anzeigen, sind mit [\textit{Ende}] markiert, und bei den Nebensätzen ist nach \textit{NS} die Art des Nebensatzes angegeben: \textit{Ang} für Angabensätze, \textit{Erg} für Ergänzungssätze und \textit{Rel} für Relativsätze.
...
Dies kann nur funktionieren,[NS Ang] wenn von außen Energie in das System gebracht wird.
...
Die gesamte Maschine muss sehr klein sein (wenige Mikrometer),[NS Ang] damit die Stöße des umgebenden Gases keinen nennenswerten Einfluss auf sie haben.
...
Ein Teilchen,[NS Rel] das wie durch den roten Pfeil markiert auftrifft,[Ende:NS Rel] bewirkt keine Drehung,[NS Ang] da der Sperrzahn die Ratsche blockiert.
...
Der Sperrzahn funktioniert nur,[NS Ang] wenn er mit einer Feder gegen die Ratsche gedrückt wird.
Auch er unterliegt dem Bombardement der Brownschen Molekularbewegung.
Wird er durch diese ausgelenkt,[NS Ang] beginnt er auf die Ratsche zu schlagen,[NS Rel] was zu einem Nettodrehmoment entgegen der zuvor angenommen Drehrichtung führt.
Die Wahrscheinlichkeit für die Auslenkung des Sperrzahns,[NS Rel] die groß genug ist,[Inf] um eine Ratschenposition zu überspringen,[Ende:Inf] ist $exp(-\Delta E/k_BT)$,[NS Ang] wobei $\Delta E$ die Energie,[NS Rel] die benötigt wird,[Inf] um die Feder des Sperrzahns auszulenken,[Ende:Inf] ist,[Koord] $T$ die Temperatur und $k_B$ die Boltzmann-Konstante.
...
Anders sieht es aus,[Ende:NS Ang] wenn ein Temperaturunterschied zwischen Flügelscheibe und Ratsche vorliegt.
Ist die Umgebung des Flügelrades wärmer als die der Ratsche,[NS Ang] dreht sich die molekulare Ratsche wie zuvor angenommen.
Ist die Umgebung der Ratsche wärmer,[Ende:NS Ang] dreht sich die Maschine in die entgegengesetzte Richtung.
\end{quote}
\end{sloppypar}

In den letzten beiden Sätzen sind die Angabensätze vorangestellt, und es gibt daher nur eine Markierung ihres rechten Randes.
