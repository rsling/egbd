...
In (\ref{ex:konstituentenstellunginunabhaengigensaetzen017}) sieht man an den beispielhaften Umformungen eines eingeleiteten Nebensatzes (\ref{ex:konstituentenstellunginunabhaengigensaetzen017a}) in verschiedene uneingeleitete Sätze (also Hauptsätze), welche zahlreichen Umstellungen möglich sind, nämlich (\ref{ex:konstituentenstellunginunabhaengigensaetzen017b})--(\ref{ex:konstituentenstellunginunabhaengigensaetzen017f}).

\begin{exe}
  \ex\label{ex:konstituentenstellunginunabhaengigensaetzen017}
  \begin{xlist}
    \ex{\label{ex:konstituentenstellunginunabhaengigensaetzen018} dass Ischariot wahrscheinlich dem Arzt das Bild verkauft hat}
...
Die Optionen der Voranstellung aus (\ref{ex:konstituentenstellunginunabhaengigensaetzen017}) werden durch die Voranstellung von komplexeren Satzteilen erweitert, von denen einige in (\ref{ex:konstituentenstellunginunabhaengigensaetzen0172}) gezeigt werden.
...
\begin{figure}[!htbp]
  ...
  \begin{forest}
    ...
    [, phantom, l sep+=2em
      ...
      {\draw ($(Vfterm.west |- Vfnode.north) + (-0.2,0.3)$) -- ($(Vfterm.east |- Vfnode.north) + (0.2,0.3)$) -- ($(Vfterm.south east) + (0.2,0)$) -- node [midway, below] {Vf} ($(Vfterm.south west) + (-0.2,0)$) -- cycle;}
      {\draw ($(Lskterm.west |- Vfnode.north) + (0,0.3)$) -- ($(Lskterm.east |- Vfnode.north) + (0,0.3)$) -- ($(Lskterm.east |- Vfterm.south) + (0,0)$) -- node [midway, below] {LSK} ($(Lskterm.west |- Vfterm.south) + (0,0)$) -- cycle;}
      {\draw ($(Mffirstterm.west |- Vfnode.north) + (-0.2,0.3)$) -- ($(Mflastterm.east |- Vfnode.north) + (0.2,0.3)$) -- ($(Mflastterm.east |- Vfterm.south) + (0.2,0)$) -- node [midway, below] {Mf} ($(Mffirstterm.west |- Vfterm.south) + (-0.2,0)$) -- cycle;}
      {\draw ($(Rskfirstterm.west |- Rsknode.north) + (0,0.3)$) -- ($(Rsklastterm.east |- Rsknode.north) + (0,0.3)$) -- ($(Rsklastterm.east |- Vfterm.south) + (0,0)$) -- node [midway, below] {RSK} ($(Rskfirstterm.west |- Vfterm.south) + (0,0)$) -- cycle;}
    ]
  \end{forest}
...
\begin{exe}
  \ex{\label{ex:verberstsaetze081} Verkaufe das Bild.}
\end{exe}
...
\Uebung[\tristar] \label{u125} Analysieren Sie die folgenden NPs mit Relativsatz nach dem Muster von Abbildung~\ref{fig:relativsaetze097} (s.\ S.~\pageref{fig:relativsaetze097}), aber ohne Kongruenz- und Rektionspfeile.
...
\Uebung[\tristar] \label{u126} Dialektal gibt es Relativsätze bzw.\ eingebettete \textit{w}"=Sätze wie in (\ref{ex:adverbialsaetze185}).
