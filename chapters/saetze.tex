...
In (\ref{ex:satz08154711}) ist zur Illustration die Matrix jeweils in [~]\Sub{M} und die eingebettete Konstituente in [~]\Sub{E} eingeklammert.
Die Interpunktion wird zugunsten der Übersichtlichkeit weggelassen.

\begin{exe}
  \ex\label{ex:satz08154711}
  \begin{xlist}
    \ex{\label{ex:satz08154711a} [Der Arzt weiß [dass Ischariot Maler ist]\Sub{E} ]\Sub{M}}
    \ex{\label{ex:satz08154711b} Der Arzt kennt [den Mann [der die Bilder gemalt hat]\Sub{E} ]\Sub{M}}
  \end{xlist}
\end{exe}
...
\begin{exe}
  \ex{\label{ex:satz08154712} [das Pferd [mit der schönsten Mähne]\Sub{E} ]\Sub{M}}
\end{exe}
...
Aus Abbildung~\ref{fig:vgreinstelligwh} ergeben sich gemäß diesen Anweisungen die Möglichkeiten in den Abbildungen~\ref{fig:movev2a}--\ref{fig:movev2d}.
...
Manche Sprecher akzeptieren Strukturen wie in (\ref{ex:saetze1992a}) allerdings doch, wenn der Kasus der Relativphrase obliquer ist als der durch Rektion im Matrixsatz geforderte, vgl.\ (\ref{ex:saetze8282821a}).
Wenn die Relativphrase in einem weniger obliquen Kasus steht, funktioniert das allerdings nicht, wie in (\ref{ex:saetze8282821b}).
