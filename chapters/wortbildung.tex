...
In den Gruppen in (\ref{ex:kompositionstypen002}) und (\ref{ex:kompositionstypen003}) bildet der Kopf (das rechte Glied) das semantische Zentrum auf eine charakteristische Weise.
...
\Dirty%
In Komposita aus zwei Substantiven gibt es in vielen, aber nicht allen Fällen eine morphologische Markierung, die an der so genannten \textit{Fuge} (der Grenze zwischen den beiden Gliedern des Kompositums) auftritt.
Betrachtet man Wörter wie \textit{Bäll-e.sammlung}, so sieht man, dass das Suffix \textit{-e} an das Vorderglied \textit{Ball} angefügt wird.
In diesem Fall löst es zusätzlich Umlaut aus.
Hier ist das so genannte \textit{Fugenelement} \textit{\char`~e} also formal identisch mit der Pluralmarkierung des Wortes \textit{Ball}.
Abbildung~\ref{fig:kompositionsfugen011} zeigt beispielhaft die Analyse eines Kompositums mit Fugenelement.
...
\begin{figure}[!htbp]
  ...
  \end{forest}
  \caption{Kompositionsstrukturen mit Fugenelement am Beispiel von \textit{Bällesammlung}}
...
\Dirty%
Der häufigste Fall ist die einfache Aneinanderfügung der beiden Substantive (die sogenannte \textit{Nullfuge}), aber es gibt elf weitere (teilweise sehr seltene) Verfugungen.
Das Erstglied hat einen erheblichen Einfluss darauf, welche Fugen möglich oder präferiert sind, und wir diskutieren nur Erstglieder.
...
Substantive jedoch, die den Plural auf \textit{-s} bilden (\textit{Fotos}, \textit{Tees}, \textit{Teams} usw.) verfugen niemals mit \textit{-s}.
...
Diese Feminina haben in ihrem Flexionsparadigma jeweils überhaupt keine Form mit \textit{-s}.
Ähnliche nicht abgeleitete Fälle mit \textit{-}(\textit{e})\textit{s}, die keiner Kasus-Numerus-Form des Erstglieds entsprechen, finden wir zum Beispiel in \textit{Schmerz-ens.geld} oder \textit{Heirat-s.antrag}.
Manche produktiven oder historischen Wortbildungssuffixe werden an der Kompositionsfuge gelöscht.
Beispielsweise entfällt in den typischen femininen Substantiven wie \textit{Wolle} oder \textit{Kirsche}, die auf Schwa enden, oft das Schwa (\textit{Woll.decke}, \textit{Kirsch.kuchen}).
\textbf{(Ulrike: War fem. -e jemals innergermanisch produktiv?)}

\Dirty%
Es herrscht Einigkeit darüber, dass Fugenelemente niemals Kasus markieren, auch wenn sie formal identisch zum Genetiv sind wie im Fall der starken und gemischten Maskulina und Neutra.
...
Diese Argumentation ist für sich genommen insofern nicht unbedingt überzeugend, als man entgegnen könnte, dass die hypothetischen genetivischen Fugenelemente gerade ein Gegenbeispiel gegen solche Theorien sind.
Es gibt aber eine Reihe von funktionalen Argumenten.
Erstens ist das Fugen-\textit{}(\textit{e})\textit{s} auf viele Fälle (bei den abgeleiteten Substantiven sogar sehr systematisch) ausgedehnt worden, in denen es rein formal kein Genitiv sein kann.
...
eine \textit{Gartentür} ist eine Tür \textit{zu} einem Garten, eine \textit{Anfangszeit} ist eine Zeit, \textit{während} derer etwas angefangen hat, ein \textit{Kirschkuchen} ist ein Kuchen \textit{aus} Kirschen bzw.\ aus Kirsch(material), eine \textit{Katzenpfote} ist ein Pfote \textit{als Körperteil} einer Katze, ein \textit{Kinderbuch} ist ein Buch \textit{für} Kinder usw.
Die meisten von diesen Relationen lassen sich auch durch einen Genitiv ausdrücken, weil der Genitiv zwischen zwei Substantiven selber semantisch sehr unspezifisch ist.
Oft, aber nicht immer können wir diese Komposita durch einen Genitiv paraphrasieren:
...
Sie korreliert auch nicht mit der Paraphrasierbarkeit des Kompositums mit einem selbständigen Genitiv.
Es gibt also keinen Grund, anzunehmen, dass bei den genitivähnlichen Fugen auch eine Genitivfunktion vorliegt.

\Dirty%
Eine größere Gruppe von Fugen, die in Tabelle~\ref{tab:kompositionsfugen010} besonders markiert wurden, ist schließlich mit den Pluralen des Erstglieds identisch.
Diese Formen der Erstglieder haben nicht zuverlässig Pluralbedeutung.
...
Wenn das Zweitglied semantisch etwas bezeichnet, das eine pluralische oder kollektive Bedeutung erzwingt, dann steht das Erstglied präferiert mit Pluralfuge.
Zweitglieder, die eine solche Bedutung haben, sind zum Beispiel Kollektivbezeichnungen wie \textit{Haufen} oder \textit{Gruppe}, Bezeichnungen für wechselseitige Relationen wie \textit{Distanz} oder \textit{Austausch} und eingeschränkt (nur unter semantischen und kontextuellen Zusatzbedingungen) Behälter-\slash Sammelbezeichnungen wie \textit{Kiste} oder \textit{Katalog}.
...
\Dirty%
Das Phänomen der Pluralfugen ist aus Sicht der systemtischen Grammatik insofern schwierig, als es keine klar benennbare formale Regularität gibt, wann die Pluralfuge stehen muss und wann sie auf jeden Fall Pluralbedeutung hat.
...
Die andere Fuge wird dadurch aber nicht ungrammatisch, sondern nur ungewöhnlich.
Dass die Generalisierung \textit{weich}\label{abs:weich} (also nicht kategorisch im Sinne eines Entweder-Oder) ist, erkennt man auch daran, dass sich in \citet{SchaeferPankratz2018} gezeigt hat, dass der Effekt umso stärker ist, je eindeutiger die entsprechende Fuge als Pluralkennzeichen erkennbar ist.
...
Bei Adjektiven gilt, dass es nur bei solchen Adjektiven voll produktiv ist, die selbst einem erkennbaren Muster der Adjektivbildung folgen, vgl.\ (\ref{ex:derivationohnewortklassenwechsel021}) und (\ref{ex:derivationohnewortklassenwechsel0212}).
