...
\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}[every text node part/.style={align=center}]
    \node (UeVok) at (0,2)  {\textbf{Vokale}\\(protoyp.\\silbisch)};
    \node (UeApr) at (2.5,2)  {Approximanten};
    \node (UeNas) at (5,2)  {Nasale};
    \node (UePlo) at (7.5,2)  {Plosive};
    \node (UeFri) at (10,2)  {Frikative};
    \node (UeAfr) at (12.5,2) {Affrikaten};

    \node (UeSon) at (2.5,4) {\textbf{Sonoranten}\\(Klanglaute)};
    \node (UeObs) at (10,4) {\textbf{Obstruenten}\\(Geräuschlaute)};

    \node (UeKon) at (7.5,0) {\textbf{Konsonanten}\\(prototyp.\\nicht silbisch)};
...
Hier wird aber von \textit{Endranddesonorisierung} gesprochen, da es der präzisere Begriff ist.
...
Am Wortende gibt es aber wegen der Endranddesonorisierung nur stimmloses \textipa{[s]} (\textit{Bus} usw.), im Wortanlaut dafür immer nur stimmhaftes \textipa{[z]} (\textit{Sahne} usw.).
...
\Zusammenfassung{%
  Im Deutschen sind Obstruenten im Silbenauslaut immer stimmlos (Endranddesonorisierung).
...
\Uebung[\tristar] \label{u34} In Abschnitt~\ref{sec:palatoalveolareundalveolare} wird behauptet, dass Wörter wie \textit{Garage} und \textit{Genie} nicht zum Kernwortschatz gehören, weil sie ein \textipa{[Z]} enthalten.
